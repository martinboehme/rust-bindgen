...
/// A trait to convert a rust type into a reference, optionally const, to the same
/// type.
trait ToRef {
    fn to_ref(self, is_const: bool) -> proc_macro2::TokenStream;
}

impl ToRef for proc_macro2::TokenStream {
    fn to_ref(self, is_const: bool) -> proc_macro2::TokenStream {
        if is_const {
            quote! { & #self }
        } else {
...
impl<'a> FieldCodegen<'a> for BitfieldUnit {
        ...
        M: Extend<proc_macro2::TokenStream>,
    {
        ...
        for bf in self.bitfields() {
            ...
            let bitfield_ty =
                bitfield_ty.to_rust_ty_or_opaque(ctx, bitfield_ty_item).unwrap_ts();
    ...
    fn codegen<'a>(
        ...
        if !is_opaque {
            ...
            for base in self.base_members() {
                ...
                let mut inner = inner_item.to_rust_ty_or_opaque(ctx, &()).unwrap_ts();
...
impl RustTy {
    ...
    // TODO ADE - remove all calls to this
        ...
        match *self.kind() {
            ...
            TypeKind::Pointer(inner) | TypeKind::Reference(inner) => {
                ...
                if inner_ty.canonical_type(ctx).is_function() || is_objc_pointer
                {
                    ...
                    Ok(RustTy::new_reference(ty))
                } else {
                    Ok(ty.to_ptr(is_const).into())
                }
            }
            TypeKind::TypeParam => {
                let name = item.canonical_name(ctx);
