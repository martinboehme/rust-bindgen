...
impl Type {
    ...
    pub fn get_associated_type_name(&self) -> Option<String> {
        // This is terrible :(
        fn hacky_parse_associated_type<S: AsRef<str>>(spelling: S) -> Option<String> {
            lazy_static! {
            ...
            ASSOC_TYPE_RE.captures(spelling.as_ref()).map(|caps| caps.get(1).unwrap().as_str().to_string())
        }

        if self.kind() != CXType_Unexposed {
            return None;
        }
        let r = hacky_parse_associated_type(self.spelling()).or_else(|| hacky_parse_associated_type(self.canonical_type().spelling()));
        eprintln!("ADE: get_associated_type_name={:?}", r);
        r
    }
}
